...
SYSTEM_PROMPT = """คุณคือผู้ช่วยให้ข้อมูลด้านภัยออนไลน์ การหลอกลวงทางไซเบอร์ และการป้องกันตัว ตอบโดยอ้างอิงจาก "ข้อมูลอ้างอิง" ที่ให้มาเท่านั้น
...
4. ใช้ภาษาสุภาพ ตรงไปตรงมา ไม่ตัดสินผู้ที่ตกเป็นเหยื่อ
5. ถ้าผู้ถามกำลังเสียเงินหรือเพิ่งโอนเงินไป ให้แจ้งขั้นตอนเร่งด่วนก่อน คือโทรอายัดกับธนาคารและโทร 1441 ทันที
6. ตอบกระชับ ไม่เกิน 5-6 ประโยค"""

# หัวข้อใน prompt — ประกาศเป็นค่าคงที่ เพราะ generator.NoLLM ต้องใช้ตัวเดียวกัน
# ในการตัดเนื้อหากลับออกมา ถ้าแก้ข้อความตรงนี้ อีกฝั่งจะตามไปเองอัตโนมัติ
CONTEXT_HEADER = "ข้อมูลอ้างอิง:"
QUESTION_HEADER = "คำถามของผู้ใช้:"

USER_PROMPT = f"""{{history}}{CONTEXT_HEADER}
{{context}}
...
# --------------------------------------------------- query transform
REWRITE_PROMPT = """แปลงคำถามให้ชัดเจนและเหมาะกับการค้นหาในฐานข้อมูลภัยออนไลน์และการป้องกันตัว
- แก้คำสะกดผิด แทนคำแสลงด้วยศัพท์ทางเทคนิคที่ใช้ในเอกสาร
...
HYDE_PROMPT = """เขียน "คำตอบสมมติ" สำหรับคำถามนี้ ในสำนวนเดียวกับบทความให้ความรู้ด้านความปลอดภัยไซเบอร์
